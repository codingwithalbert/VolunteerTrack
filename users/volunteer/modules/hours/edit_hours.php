...
// Fetch hour entry (only if pending and belongs to user)
$query = "SELECT * FROM hours WHERE hour_id = :hour_id AND volunteer_id = :user_id AND status = 'pending'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($activity_id) || empty($hours_worked) || empty($work_date)) {
        ...
        $query = "UPDATE hours SET activity_id = :activity_id, hours_worked = :hours_worked, 
                  work_date = :work_date, description = :description 
                  WHERE hour_id = :hour_id AND volunteer_id = :user_id AND status = 'pending'";
        $stmt = $db->prepare($query);
...
<body>
    ...
    <div class="container">
        ...
        <div class="info-box">
            <p><strong>Note:</strong> You can only edit hours that are pending verification. Once verified or rejected, entries cannot be modified.</p>
        </div>
        ...
        <?php if ($success): ?>
            <div class="alert alert-success">
                <?php echo $success; ?>
                <a href="manage_hours.php">View all my hours</a>
            </div>
        <?php endif; ?>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" 
                              placeholder="Describe what you did during this volunteer session..."><?php echo htmlspecialchars($hour['description']); ?></textarea>
                </div>
